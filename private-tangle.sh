...
setupCoordinator () {
  ...
  docker-compose run -d --rm -e COO_SEED=$COO_SEED coo hornet --overwriteCooAddress --cooBootstrap --cooStartIndex 65536 > coo.bootstrap.container
  ...
  docker logs $(cat ./coo.bootstrap.container) 2>&1 | grep "milestone issued (65536)"
  bootstrapped=$?
